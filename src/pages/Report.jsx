...
//       {candidates.length > 0 && (
//         <button onClick={handleDownloadPDF} className="mt-10 bg-purple-700 text-white font-bold py-3 px-6 rounded-lg">
...
export default function Report() {
  ...
  useEffect(() => {
    const loadData = async () => {
      const data = await fetchParsedResumes();
      if (data.length > 0) {
        // Ensure each candidate has at least top 5–6 job matches
        const processed = data.map((candidate) => ({
          ...candidate,
          jobMatches: candidate.jobMatches?.slice(0, 6) || [],
        }));
