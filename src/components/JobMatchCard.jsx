...
const JobMatchCard = ({ match }) => {
  return (
    <div className="bg-gradient-to-r from-green-50 via-white to-yellow-50 p-4 rounded-2xl 
                    shadow-md hover:shadow-xl transition-all duration-300 mb-3">
      <div className="flex justify-between items-center mb-2">
        <h3 className="font-bold text-md text-green-700">{match.jobTitle}</h3>
        <span className="font-semibold text-green-800">{(match.score * 100).toFixed(1)}% Match</span>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        {match.skills?.map((skill, idx) => (
          <span
            key={idx}
            className="bg-gradient-to-r from-green-400 via-yellow-400 to-orange-400 
      ...
      <div className="w-full bg-gray-200 h-2 rounded-full overflow-hidden mt-1">
        <div
          className="h-2 bg-green-500 rounded-full transition-all duration-700"
          style={{ width: `${(match.score * 100).toFixed(1)}%` }}
        ></div>
      </div>
    </div>
  );
